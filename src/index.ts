...
function cruModeValidator<T extends Record<any, CRUSingleDeclaration>, Mode extends CRUInput>(
	declaration: T,
	mode: Mode
): CRUModeValidatorForDeclaration<T, Mode> {
 ...
 */
export function optionalCreate<T extends z.ZodTypeAny, D extends z.infer<T> | undefined = undefined>(
	validator: T,
	defaultValue?: D | (() => D)
): { read: T; create: D extends undefined ? z.ZodOptional<T> : z.ZodDefault<T>; update: z.ZodOptional<T>; } {
	return {
		read: validator,
...
export function mapValues<T extends Record<string, any>, R>(
	obj: T,
	mapper: (val: T[keyof T], key: keyof T) => R
): Record<keyof T, R> {
